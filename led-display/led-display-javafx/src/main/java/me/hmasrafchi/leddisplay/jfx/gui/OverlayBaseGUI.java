...
class OverlayBaseGUI extends BorderPane {
	...
	private final static BiMap<State, String> STATE_TO_BUTTONTEXT_MAPPING = HashBiMap.create();
	static {
		STATE_TO_BUTTONTEXT_MAPPING.put(State.ON, "X");
		STATE_TO_BUTTONTEXT_MAPPING.put(State.OFF, "O");
		STATE_TO_BUTTONTEXT_MAPPING.put(State.TRANSPARENT, "");
	}

	private ButtonGrid buttonGrid;

	OverlayBaseGUI(final List<List<State>> states) {
	...
	private ButtonGrid getStatesButtonGrid(final List<List<State>> states) {
	...
	private void setButtonTextBasedOnState(final ButtonGrid buttonGrid, final List<List<Overlay.State>> states,
			final BiMap<State, String> stateToButtonTextMapping) {
		for (int i = 0; i < states.size(); i++) {
			for (int j = 0; j < states.get(0).size(); j++) {
				final State state = states.get(i).get(j);
	...
	private void buttonActionHandler(final Button button, final BiMap<State, String> stateToButtonTextMapping) {
		...
		final State currentState = STATE_TO_BUTTONTEXT_MAPPING.inverse().get(currentButtonText);
