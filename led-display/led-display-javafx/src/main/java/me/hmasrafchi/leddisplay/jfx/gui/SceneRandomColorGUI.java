...
public final class SceneRandomColorGUI extends BorderPane implements Model<Scene> {
	...
	public SceneRandomColorGUI(final List<Led.RgbColor> colors) {
	...
	private List<ColorPicker> initializeColorPickers(final List<Led.RgbColor> colors) {
	...
	public Scene getModel() {
		return new SceneRandomColor(getColors());
	}
}